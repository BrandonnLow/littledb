...
func TestRaftLogAppendAndRead(t *testing.T) {
	...
	i1 := l.append(currentTerm, []byte("one"))
	i2 := l.append(currentTerm, []byte("two"))
	if i1 != 1 || i2 != 2 {
	...
	if l.term(1) != currentTerm || l.term(2) != currentTerm {
		t.Errorf("terms = %d,%d, want %d,%d", l.term(1), l.term(2), currentTerm, currentTerm)
	}
	if l.has(3) {
...
func TestRaftLogMatchesPrev(t *testing.T) {
	...
	l.append(currentTerm, []byte("a"))
	l.append(currentTerm, []byte("b"))

	if !l.matchesPrev(2, currentTerm) {
		t.Error("matchesPrev(2, currentTerm) = false, want true")
	}
	if l.matchesPrev(2, currentTerm+1) {
		t.Error("matchesPrev with wrong term = true, want false")
	}
	if l.matchesPrev(3, currentTerm) {
...
func TestRaftLogTruncateFrom(t *testing.T) {
	l := NewRaftLog()
	for i := 0; i < 4; i++ {
		l.append(currentTerm, []byte{byte('0' + i)})
	}
	if l.lastIndex() != 4 {
	...
	if idx := l.append(currentTerm, []byte("new")); idx != 3 {
